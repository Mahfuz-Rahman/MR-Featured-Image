...
function renderMediaUploader( $ ) {
     ...
     */
     ...
     /**
      * If we're this far, then an instance does not exists, so we need to
      * create our own.
      *
     ...
     file_frame.on( 'insert', function() {
        ...
        // Store the image's information into the data fields
 ...
 * anchor in its meta box.
 *
 * Resets the meta box by hiding the image and by hiding the 'Remove Footer Image'
 * container.
 *
...
function resetUploadForm( $ ) {
    ...
    // Finally, we add the 'hidden' class back to this anchors point
    $('#featured-footer-image-container')
        .next()
        .hide()
        .addClass('hidden');
 ...
 * If so, then the image and the 'Remove the footer featured image' anchor are displayed.
 *
 * Otherwise, the standard anchor is renderd.
 *
 * @param   object  $   A reference to the jQuery object.
 * @since   0.1.0
 */
function renderFeaturedImage( $ ) {
     ...
     * Then we need to display the image and the rest link.
     */
...
(function( $ ){
    ...
    jQuery(document).ready(function($) {
        ...
        $( '#remove-footer-thumbnail').on('click', function( evt ) {
            ...
            //Stop the anchor default behaviour
            evt.preventDefault();
